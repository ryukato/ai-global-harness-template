...
Some coding-agent features require API keys in the developer's local runtime.
...
These examples are placeholders. Keep project-approved variable names in this
document or in `docs/architecture/build-and-run.md`, but never include real
values.
...
Before running commands that require API keys, each developer should verify:
...
Record non-secret project settings here:
