...
This repository provides harness instructions, skills, and command prompts. It
does not provide shared MCP credentials.
...
Each developer who wants agent commands to read Jira, Confluence, GitHub issues,
or pull requests must configure the matching connector, MCP server, or CLI
authentication in their own local agent runtime.
...
Commands that depend on this setup include:

```text
/project:cawf:plan <jira-url>
/project:cawf:plan <github-issue-url>
/project:cawf:git <issue-or-task>
```
...
Before using Jira or GitHub backed agent commands, each developer should verify:
...
Record non-secret project settings here or in nearby operation docs:
...
- Redact sensitive values before writing agent outputs to `.ai-workspace`.
