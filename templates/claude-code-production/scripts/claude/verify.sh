...
DOCS_DIR="${HARNESS_DOCS_DIR:-docs/operations}"
ROOT_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"
cd "$ROOT_DIR"

FAILED=0

PROFILE_FILE="${HARNESS_PROFILE_FILE:-$DOCS_DIR/harness-profile.env}"
HARNESS_PROFILE="auto"

if [ -f "$PROFILE_FILE" ]; then
  # shellcheck disable=SC1090
  source "$PROFILE_FILE"
elif [ -f "docs/$AGENT_NAMESPACE/harness-profile.env" ]; then
  PROFILE_FILE="docs/$AGENT_NAMESPACE/harness-profile.env"
  # shellcheck disable=SC1090
  source "$PROFILE_FILE"
fi
...
has_package_script() {
  ...
  if command -v jq >/dev/null 2>&1; then
    jq -e --arg script "$script_name" '.scripts[$script] // empty' "$package_dir/package.json" >/dev/null 2>&1
    return $?
  fi

  grep -Eq "\"$script_name\"[[:space:]]*:" "$package_dir/package.json"
}
...
node_script_requires_bin() {
  ...
  if command -v jq >/dev/null 2>&1; then
    jq -e --arg script "$script_name" --arg bin "$bin_name" '.scripts[$script] | strings | contains($bin)' "$package_dir/package.json" >/dev/null 2>&1
    return $?
  fi
...
run_auto_detected_checks() {
  ...
  if [ -f "gradlew" ] || [ -f "build.gradle" ] || [ -f "build.gradle.kts" ] || [ -f "settings.gradle" ] || [ -f "settings.gradle.kts" ] || [ -f "mvnw" ] || [ -f "pom.xml" ]; then
    detected=true
  fi
  run_jvm_auto_checks

  if [ "$detected" != "true" ]; then
    echo "No code stack detected for profile '$HARNESS_PROFILE'." >&2
    echo "Set HARNESS_PROFILE=docs-only in $PROFILE_FILE until code is added, or add package/build files." >&2
    mark_fail
  fi
}
